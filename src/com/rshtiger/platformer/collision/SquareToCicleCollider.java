...
public class SquareToCicleCollider extends Collider {
	double chkX, chkY;
	@Override
	public boolean isCollided (Shape player, Shape s) {
		if (getCenterX(s) < getCenterX(player) - getLengthX(player)/2.0f)
			chkX = getCenterX(player) - getLengthX(player)/2.0f;
		else if ( getCenterX(s) > getCenterX(player) + getLengthX(player)/2.0f )
			chkX = getCenterX(player) + getLengthX(player)/2.0f;
		else
			chkX =  getCenterX(s) ;

		if (getCenterY(s) < getCenterY(player) - getLengthY(player)/2.0f)
			chkY = getCenterY(player) - getLengthY(player)/2.0f;
		else if ( getCenterY(s) > getCenterY(player) + getLengthY(player)/2.0f )
			chkY = getCenterY(player) + getLengthY(player)/2.0f;
		else
			chkY =  getCenterY(s);
